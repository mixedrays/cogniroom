...
const TOOLS = [askUserTool, readyToGenerateTool];
...
export function WizardAgentDialog({
  ...
  const { messages, isStreaming, sendMessage, submitToolResult, dismissToolCall } =
  ...
  const handleSubmit = (text: string, _model: string) => {
    setInput("");
    sendMessage(text);
  };
