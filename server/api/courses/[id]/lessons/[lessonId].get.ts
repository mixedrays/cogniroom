...
export default defineEventHandler(
  withErrorGuard("Failed to load lesson content", async (event) => {
    ...
    const result = await courseRepo.getLessonContent(
      storageApi,
      courseId,
      lessonId
    );
    if (!result) {
      throw new HTTPError({ status: 404, message: "Lesson content not found" });
    }
    return result;
  })
);
