...
describe("mermaid markdown rendering", () => {
  ...
  it("renders mermaid diagrams without console errors", async () => {
    ...
    try {
      ...
      expect(svgCount).toBe(MERMAID_DIAGRAM_COUNT);

      // Verify no diagram is in the error state
      ...
      writeFileSync(
        join(ARTIFACTS_DIR, `${timestamp}-mermaid.json`),
